...
load("Desktop/Proyecto Pinzones 2020/Data.Paola.Rdata")
glimpse(Data.Paola)
...
#data<-read.csv("example1.data.csv",header=T)
taxa<-as.factor(data.fortis[,10])
level<-as.factor(data.fortis[,8])
Y<-as.matrix(data.fortis[,(5:7)])	## Removes factor columns from rest of data
Y1 <-na.omit(Y)
taxaBylevel<-as.factor(paste(taxa,level))  #for LS means at each level
Y<-prcomp(Y1)$x	#TO USE IF > 2 variables: for plotting PC axes at the end
PCAPlot <- biplot(prcomp(Y))

#Basic data parameters (n=#spec, p=#ldmk, k=#dim)
n<-length(levels(taxa)) #was n.taxa
p<-length(levels(level))  #was n.level
k<-ncol(Y1) #dimensions of phenotypic data

lm.full<-lm(Y1~taxa*level,model=T,x=T,y=T,qr=T) 
yhat.full<-predict(lm.full)
summary(manova(lm.full))
print("Manova summary stats")
print(summary(manova(lm.full), test = "Wilks"))

#For resid randomization (used later)
lm.red<-lm(Y1 ~ taxa + level, model=T,x=T,y=T,qr=T)   #just the covariate
